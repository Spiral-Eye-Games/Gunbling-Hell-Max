...
static func _try_hit_enemies(state: RunState, bullet: Dictionary, bullet_index: int, game: Node) -> bool:
	...
	while ei >= 0:
		...
		if bullet.already_hit.has(enemy.id):
			ei -= 1
			continue
		if CollisionUtils.circles_overlap(bullet.x, bullet.y, bullet.radius, enemy.x, enemy.y, enemy.radius, bullet.laser_width):
			bullet.already_hit[enemy.id] = true
			RouletteSystem.on_successful_hit(state, game)
			enemy.hp -= bullet.damage
			game.add_hit_effect(enemy.x, enemy.y, bullet.color, 22.0 if bullet.piercing else 16.0)
			var enemy_id = enemy.id
			if bullet.explosive:
				var missile_tokens: int = int(state.current_magazine.get("missile_tokens", 0))
				_explode(state, bullet.x, bullet.y, bullet.explosion_radius, 1.6 + missile_tokens, game)
			if enemy.hp <= 0:
				EnemySystem.kill_enemy_by_id(state, enemy_id, game)
			if not bullet.piercing and not bullet.explosive:
				state.bullets.remove_at(bullet_index)
				return true
			if bullet.explosive and not bullet.piercing:
				state.bullets.remove_at(bullet_index)
				return true
		ei -= 1
	return false
...
static func _try_hit_boss(state: RunState, bullet: Dictionary, bullet_index: int, game: Node) -> void:
	...
	if not CollisionUtils.circles_overlap(bullet.x, bullet.y, bullet.radius, boss.x, boss.y, boss.radius, bullet.laser_width):
		return
	bullet.already_hit["boss"] = true
	RouletteSystem.on_successful_hit(state, game)
	boss.hp -= bullet.damage
	state.score += int(floor(100.0 * state.combo))
	game.add_hit_effect(boss.x, boss.y, bullet.color, 28.0 if bullet.piercing else 22.0)
	if bullet.explosive:
		var missile_tokens: int = int(state.current_magazine.get("missile_tokens", 0))
		_explode(state, bullet.x, bullet.y, bullet.explosion_radius, 2.5 + missile_tokens, game)
	if not bullet.piercing or bullet.explosive:
		state.bullets.remove_at(bullet_index)
	if boss.hp <= 0:
		state.boss = null
		state.phase = "win"
...
static func _explode(state: RunState, x: float, y: float, radius: float, damage: float, game: Node) -> void:
